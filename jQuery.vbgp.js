/*	Visual Bootstrap grid by PixelSPB, v. 1.1 © 2018, All rights reserved 
...
jQuery(document).ready(function($) {
	...
	if(typeof(Storage)!=="undefined"){
		storageStatus = true;
		if(sessionStorage['showPixelGrid'] == 'true'){
			clickedCheckboxVisualGrid('showPixelGrid', true);
		}
		if(sessionStorage['showPixelGridPaddings'] == 'true'){
			clickedCheckboxVisualGrid('showPixelGridPaddings', true);
		}
		if(sessionStorage['showPixelGridColNum'] == 'true'){
			clickedCheckboxVisualGrid('showPixelGridColNum', true);
		}
		if(sessionStorage['showPixelGridGrayScale'] == 'true'){
	...
	function clickedCheckboxVisualGrid(idCheckboxVisualGrid, keyPressStatus){
		...
		if ($('#'+idCheckboxVisualGrid).is(':checked')){
			sessionStorageMemory(idCheckboxVisualGrid, true);
		} else {
			sessionStorageMemory(idCheckboxVisualGrid, false);
		}
	...
	function sessionStorageMemory(idVisualGrid, currentStatus){
		if (storageStatus){
			sessionStorage[idVisualGrid] = currentStatus;
			console.log(idVisualGrid);
		}
	}
